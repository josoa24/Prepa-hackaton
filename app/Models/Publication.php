...
class Publication extends Model
{
  ...
  protected $allowedFields    = ['id_user', 'title', 'content', 'description', 'date_publication', 'date_evenement', 'created_at', 'type', 'forme'];
  ...
  public function getPublicationsWithPhotosAndStatusSearch($page, $limit, $search)
  {
    $query = $this->select('Publication.*, photos.lien AS photo_link, 
                                user.first_name, user.last_name, user.email, user.profile_picture,
                                (CASE 
    ...
    if ($limit !== 0) {
      $query->limit(intval($limit), intval($page));
    }
